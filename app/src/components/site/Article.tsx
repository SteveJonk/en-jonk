...
/** The three "Hoe wij kijken" layers, in reading order. */
export const LAYERS = [
  { href: '/hoe-wij-kijken/ik', title: <>Ik</>, bar: 'bg-rose', border: 'border-rose' },
  {
    href: '/hoe-wij-kijken/jij-en-ik',
    title: (
      <>
        Jij <Amp /> ik
      </>
  ...
  return (
    <>
      <section className='pt-32 pb-12 md:pt-44 md:pb-16'>
        <Container>
          <Reveal as='p'>
            <ArrowLink href='/hoe-wij-kijken' back>
              Hoe wij kijken
            </ArrowLink>
          </Reveal>
          <div className='mt-10 grid items-end gap-10 lg:grid-cols-12 lg:gap-14'>
            <Reveal className='lg:col-span-7'>
              <p className='eyebrow'>Drie lagen &middot; laag {layer + 1} van 3</p>
              <span className={cn('mt-5 block h-px w-10', bar)} />
      ...
      <Container>
        <Reveal>
          <Frame src={image} alt={alt} priority className='aspect-[4/3] md:aspect-[21/9]' />
        </Reveal>
      </Container>
    </>
  );
...
export function AsideImage({ src, alt }: { src: string; alt: string }) {
  return (
    <Reveal as='figure'>
      <Frame src={src} alt={alt} sizes='(min-width:1024px) 50vw, 100vw' className='aspect-[4/5]' />
    </Reveal>
  );
}
...
export function AsideFigure({ caption, children }: { caption: string; children: ReactNode }) {
  return (
...
export function ArticleFigure({ caption, children }: { caption: string; children: ReactNode }) {
  return (
...
export function ArticleOutcome({ layer, children }: { layer: Layer; children: ReactNode }) {
  return (
    <Reveal
      className={cn(
...
export function LayerNav({ current }: { current: Layer }) {
  return (
        ...
        <Reveal className='mt-6 grid gap-px border-y border-ink/10 bg-ink/10 md:grid-cols-3'>
          {LAYERS.map((layer, i) => {
            ...
            return (
              ...
              >
                <span className={cn('block h-px w-10', layer.bar)} />
