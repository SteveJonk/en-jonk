...
const DOTS = ['bg-rose', 'bg-green', 'bg-steel'];
    ...
    <div className={cn('relative', className)}>
      ...
      <ol className='grid gap-10 lg:grid-cols-3 lg:gap-8'>
        {items.map((item, i) => (
          ...
          >
            <span className={cn('hidden size-[15px] rounded-full lg:block', DOTS[i % 3])} />
            <span
              className={cn('absolute top-2 -left-[5px] size-[9px] rounded-full lg:hidden', DOTS[i % 3])}
            />
